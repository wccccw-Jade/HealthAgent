...
def _payload(text: str = "列一下我的药") -> dict:
    return {
        "schema": "2.0",
        "header": {
...
def test_confirm_and_cancel_route_to_conversation_service(monkeypatch, db_session) -> None:
    ...
    confirm_response = client.post("/feishu/webhook", json=_payload("确认"))
    cancel_response = client.post("/feishu/webhook", json=_payload("取消"))
